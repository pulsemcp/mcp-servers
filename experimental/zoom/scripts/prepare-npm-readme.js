...
try {
  ...
  const repoNotice = `> **Note**: This package is part of the [MCP Servers](https://github.com/pulsemcp/mcp-servers) monorepo. For the latest updates and full source code, visit the [Zoom MCP Server directory](https://github.com/pulsemcp/mcp-servers/tree/main/experimental/zoom).

`;

  // Insert repo notice after the title
  const titleMatch = mainContent.match(/^#.*Zoom MCP Server.*$/m);
  if (titleMatch) {
