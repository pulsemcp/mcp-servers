...
async function buildPublished() {
  ...
  try {
    ...
    // Create symlink for twist-mcp-server-shared
    const sharedPackageLink = join(nodeModulesDir, 'twist-mcp-server-shared');
    ...
    try {
      ...
      console.log('Created symlink for twist-mcp-server-shared');
    } catch (error) {
