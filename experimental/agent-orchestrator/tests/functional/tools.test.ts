...
describe('Tools', () => {
  ...
  describe('get_session', () => {
    ...
    it('should display catalog_plugins in session details', async () => {
      mockClient.getSession = vi.fn().mockResolvedValue({
        id: 1,
        slug: 'test-session',
        title: 'Test Session',
      ...
      const text = (result as { content: Array<{ text: string }> }).content[0].text;
      expect(text).toContain('**Plugins:** plugin-a, plugin-b');
    });

    it('should not display plugins line when catalog_plugins is empty', async () => {
      mockClient.getSession = vi.fn().mockResolvedValue({
        id: 1,
        slug: 'test-session',
        title: 'Test Session',
