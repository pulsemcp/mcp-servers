...
try {
  ...
  const repoNotice = `> **Note**: This package is part of the [MCP Servers](https://github.com/pulsemcp/mcp-servers) monorepo. For the latest updates and full source code, visit the [GCS MCP Server directory](https://github.com/pulsemcp/mcp-servers/tree/main/experimental/gcs).

`;

  // Insert repo notice after the title
  const titleMatch = mainContent.match(/^#.*GCS MCP Server.*$/m);
  if (titleMatch) {
