...
import { type MockGCSData } from '../../shared/src/gcs-client/gcs-client.integration-mock.js';
...
describe('GCS MCP Server Integration Tests', () => {
  ...
  describe('Resources', () => {
    it('should list available resources', async () => {
      ...
      expect(result.resources[0].uri).toBe('gcs://config');
    });

    it('should read config resource', async () => {
      client = await createTestMCPClientWithMock({});

      const result = await client.readResource('gcs://config');
      ...
      expect(config.server.name).toBe('gcs-mcp-server');
    });
  });
});

/**
 * Helper function to create a TestMCPClient with a mocked GCS client.
 */
async function createTestMCPClientWithMock(
  mockData: MockGCSData,
  ...
  const client = new TestMCPClient({
    serverPath,
    env: {
      GCS_MOCK_DATA: JSON.stringify(mockData),
      ...extraEnv,
    },
    debug: false,
  });
...
describe('GCS_BUCKET Constraint', () => {
  ...
  it('should hide bucket-level tools when GCS_BUCKET is set', async () => {
    client = await createTestMCPClientWithMock(
      { buckets: [{ name: 'test-bucket' }] },
  ...
  it('should automatically inject bucket param when GCS_BUCKET is set', async () => {
    ...
    client = await createTestMCPClientWithMock(mockData, { GCS_BUCKET: 'constrained-bucket' });
  ...
  it('should inject bucket for get_object when GCS_BUCKET is set', async () => {
    const mockData: MockGCSData = {
      buckets: [{ name: 'constrained-bucket' }],
      objects: {
        'constrained-bucket': {
  ...
  it('should include constraint note in tool description', async () => {
    client = await createTestMCPClientWithMock({}, { GCS_BUCKET: 'my-bucket' });
