...
const PARAM_DESCRIPTIONS = {
  queue_name: 'Filter by queue name (e.g., "default", "mailers")',
  status: 'Filter by job status (scheduled, retried, queued, running, finished, discarded, error)',
  job_class: 'Filter by job class name (e.g., "SendEmailJob")',
  after: 'Filter jobs created after this ISO 8601 datetime',
  before: 'Filter jobs created before this ISO 8601 datetime',
...
Use cases:
- Browse background jobs to monitor processing status
- Filter jobs by queue, status, or class
- Investigate failed or errored jobs
- Review job history within a time range`,
    inputSchema: {
      type: 'object',
      properties: {
    ...
    handler: async (args: unknown) => {
      ...
      try {
        ...
        for (const [index, job] of response.jobs.entries()) {
          ...
          if (job.created_at) {
            content += `   Created: ${new Date(job.created_at).toLocaleDateString()}\n`;
          }
          content += '\n';
        }
