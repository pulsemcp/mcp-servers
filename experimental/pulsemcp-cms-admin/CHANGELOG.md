...
- Bumped the `vitest` devDependency floor from `^3.2.3` to `^4.1.9` to match `@vitest/ui ^4.1.9`, which peer-requires `vitest@4.x`. Dependabot PR #4388 raised `@vitest/ui` to v4 without bumping `vitest`, leaving this server as the lone outlier whose `npm install` hit a deterministic `ERESOLVE` peer-dependency conflict. That conflict broke the "MCP Servers: Distribute to Public Repo" workflow's `verify-publications` CI check and blocked npm publication of `pulsemcp-cms-admin`. This change aligns the pair (and the regenerated lockfile) with every other server under `mcp-servers/servers/*`. No runtime or source code changed.
...
- All admin-API requests now flow through an internal `adminFetch` wrapper that injects the `X-PulseMCP-Internal` header from the optional `PULSEMCP_ADMIN_INTERNAL_SECRET` env var. A matching Cloudflare custom rule on `admin.pulsemcp.com` uses this header to route trusted internal traffic past the WAF and rate-limit stack, eliminating the spurious 403s that could surface under load or Super Bot Fight Mode. Without the env var set, behavior is unchanged — the request goes out with only `X-API-Key`, which is appropriate locally and against staging (staging is grey-clouded).
...
- Added `good_jobs` and `good_jobs_readonly` tool groups for GoodJob background job management
  - `list_good_jobs` - List and filter background jobs by queue, status, job class, and date range
  - `get_good_job` - Get detailed information about a specific job
  - `list_good_job_cron_schedules` - List all configured cron schedules
  - `list_good_job_processes` - List active worker processes
  - `get_good_job_queue_statistics` - Get aggregate job statistics by status
  - `retry_good_job` - Retry a failed or discarded job
  - `discard_good_job` - Discard a job to prevent retries
  - `reschedule_good_job` - Reschedule a job to a new time
