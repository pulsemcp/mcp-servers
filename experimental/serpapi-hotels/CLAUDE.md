...
- **SerpAPI Client** (`shared/src/serpapi-client/`): HTTP client for the SerpAPI REST API
  ...
  - `lib/get-hotel-reviews.ts`: Individual guest reviews with ratings and text
- **Tools** (`shared/src/tools/`): MCP tool implementations
  ...
  - `get-hotel-reviews.ts`: Get individual guest reviews with sub-ratings
...
5. User can then call `get_hotel_details` with a `property_token` from results for full details and review breakdowns
6. User can call `get_hotel_reviews` with a `property_token` for individual guest reviews (uses `engine=google_hotels_reviews`)
