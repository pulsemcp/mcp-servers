...
async function preparePublish() {
  ...
  // Copy the top-level server README into local/ so the published npm package
  // ships the user-facing README (npmjs.com renders local/README.md). Without
  // this, the published tarball would have no README despite the `files` glob
  // listing one.
  console.log('Copying README.md into local/ for npm publish...');
  await cp(join(__dirname, '../README.md'), join(__dirname, 'README.md'));
