...
if (!existsSync(envPath)) {
  ...
  console.log('   Please create .env from .env.example and add your AWS credentials:');
  console.log('   cp .env.example .env');
  console.log('   Then edit .env and add your AWS credentials');
  process.exit(1);
}

// Check if AWS credentials are set
const envContent = readFileSync(envPath, 'utf8');
if (!envContent.includes('AWS_REGION') && !envContent.includes('AWS_DEFAULT_REGION')) {
  console.error('AWS_REGION not configured in .env!');
  console.log('   Please edit .env and add your real AWS region');
  process.exit(1);
}
