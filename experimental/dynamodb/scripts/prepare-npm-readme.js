...
try {
  ...
  const repoNotice = `> **Note**: This package is part of the [MCP Servers](https://github.com/pulsemcp/mcp-servers) monorepo. For the latest updates and full source code, visit the [DynamoDB MCP Server directory](https://github.com/pulsemcp/mcp-servers/tree/main/experimental/dynamodb).

`;

  // Insert repo notice after the title
  const titleMatch = mainContent.match(/^#.*DynamoDB MCP Server.*$/m);
  if (titleMatch) {
