...
describe('AppSignal MCP Server - System Test', () => {
  ...
  it('should complete the full AppSignal workflow', async () => {
    ...
    try {
      ...
      let selectedApp = apps.find((app: any) => 
        app.name === 'pulsemcp' && app.environment === 'production'
      );
      ...
      console.log(`   ✓ Selected: ${selectedApp.name} (${selectedAppId}) - ${selectedApp.environment || 'unknown env'}`);
    ...
    try {
      ...
      if (logsResult.content[0].text.startsWith('Error')) {
        ...
        if (logsResult.content[0].text.includes('Code: 500') || logsResult.content[0].text.includes('Code: 400')) {
          outcome.warnings.push('Log search API returned an error - this may be due to API limitations or the current query structure');
        ...
        if (logsResponse.lines.length === 0) {
          outcome.warnings.push('No logs found - add some application activity to fully test log search');
    ...
    try {
      ...
      if (exceptionResult.content[0].text.includes('Error')) {
        console.log('   ✓ Exception incident endpoint works (returned expected error for non-existent ID)');
    ...
    outcome.warnings.push('Incident endpoints can only be tested with error handling - AppSignal API doesn\'t provide incident listing');
