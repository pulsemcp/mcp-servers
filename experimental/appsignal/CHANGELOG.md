...
- **BREAKING**: Shortened performance-related tool names to reduce token usage:
...
- New `truncationApplied` field in search_logs response indicates whether any messages were truncated.
- Warning message in `formattedSummary` when verbose mode is used with many results (>20) to alert about potential context overflow.

## [0.4.0] - 2025-12-09

### Added

- New `get_perf_samples` tool (originally `get_performance_samples`) for retrieving multiple recent samples for a controller action. Returns up to N samples (default 10) with detailed request info including path, params, and timing breakdown by component (active_record, action_view, etc.).
