...
import { ServerConfigGeneratorFactory } from '../llm/factory.js';
import type { ServerConfigInput, IServerConfigGenerator } from '../llm/types.js';
...
Format: [{"name": "server.name", "rationale": "why this server is needed"}]`;
  ...
  async installServers(
    ...
    try {
      ...
      // Try to get LLM-based configuration generator
      const llmGenerator = ServerConfigGeneratorFactory.createFromEnv();

      if (llmGenerator) {
        logger.debug('Using LLM-based configuration generation');
        return await this.installServersWithLLM(
          serverNames,
          allServersConfig,
          secrets,
          llmGenerator,
        ...
        logger.warn('LLM configuration not available, falling back to legacy approach');
        return await this.installServersLegacy(
          serverNames,
          allServersConfig,
          secrets,
          serverConfigs
  ...
  /**
   * Install servers using LLM-based configuration generation
   */
  private async installServersWithLLM(
    serverNames: string[],
    allServersConfig: ServerConfig[],
    secrets: Record<string, Record<string, string>>,
    llmGenerator: IServerConfigGenerator,
    serverConfigs?: Record<string, { env?: Record<string, string> }>
  ): Promise<{
    installations: Array<{
      serverName: string;
      ...
      try {
        ...
        const input: ServerConfigInput = {
          ...
          userPreferences: {
            ...
            workingDirectory: this.currentAgent.workingDir,
            ...serverConfigs?.[serverName],
          },
        };

        // Generate configuration using LLM
        const result = await llmGenerator.generateServerConfig(input);

        if (!result.success || !result.mcpConfig) {
          installations.push({
            serverName,
            status: 'failed',
            error: result.error || 'LLM configuration generation failed',
          });
          continue;
        }

        // Extract the server configuration from the generated MCP config
        const generatedConfig = result.mcpConfig as { mcpServers?: Record<string, unknown> };
        const serverSpecificConfig =
          generatedConfig.mcpServers?.[serverName] ||
          Object.values(generatedConfig.mcpServers || {})[0]; // Fallback to first server if name doesn't match

        if (!serverSpecificConfig) {
          installations.push({
            serverName,
            status: 'failed',
        ...
        const finalConfig = this.injectSecrets(serverSpecificConfig, secrets[serverName] || {});

        mcpConfig.mcpServers[serverName] = finalConfig;
        ...
        logger.debug(`Successfully generated config for ${serverName} using LLM`);
      } catch (error) {
        installations.push({
          serverName,
          status: 'failed',
          error: `LLM generation error: ${error instanceof Error ? error.message : 'Unknown error'}`,
        });
      }
    }
    ...
    logger.debug(
      `Installed ${installations.filter((i) => i.status === 'success').length} servers using LLM`
    );
  ...
  /**
   * Install servers using legacy parsing approach (fallback when LLM is not available)
   */
  private async installServersLegacy(
    _serverNames: string[],
    _allServersConfig: ServerConfig[],
    _secrets: Record<string, Record<string, string>>,
  ...
  /**
   * Inject secrets into a server configuration
   */
  private injectSecrets(serverConfig: unknown, secrets: Record<string, string>): unknown {
    ...
    // Inject secrets into headers for HTTP/SSE servers
