...
### README Concatenation and npm Publishing

- MCP servers use `scripts/prepare-npm-readme.js` to concatenate main README with local configuration sections during `prepublishOnly`
- Local README files should contain only local-specific configuration (like environment variables and setup instructions)
- The script automatically merges the main server README with local config and adds a GitHub repository reference
- Missing the concatenation script causes npm packages to publish with minimal local README instead of full documentation
- The `mcp-server-template` includes this script to prevent new servers from having this issue
- CI verification (`verify-publications`) checks for proper CHANGELOG entries when version numbers are bumped
