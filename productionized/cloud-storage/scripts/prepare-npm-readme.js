...
try {
  ...
  const repoNotice = `> **Note**: This package is part of the [MCP Servers](https://github.com/pulsemcp/mcp-servers) monorepo. For the latest updates and full source code, visit the [Cloud Storage MCP Server directory](https://github.com/pulsemcp/mcp-servers/tree/main/productionized/cloud-storage).

`;

  // Insert repo notice after the title
  const titleMatch = mainContent.match(/^#.*Cloud Storage.*$/m);
  if (titleMatch) {
