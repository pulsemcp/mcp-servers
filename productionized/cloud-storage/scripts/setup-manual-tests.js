...
if (!existsSync(envPath)) {
  ...
  console.log('   Please create .env from .env.example and add your credentials:');
  console.log('   cp .env.example .env');
  console.log('   Then edit .env and add your GCS_BUCKET and optional GCS_KEY_FILE');
  process.exit(1);
}

// Check if GCS_BUCKET is set
const envContent = readFileSync(envPath, 'utf8');
if (!envContent.includes('GCS_BUCKET=') || envContent.includes('GCS_BUCKET=your-')) {
  console.error('GCS_BUCKET not configured in .env!');
  console.log('   Please edit .env and add your GCS bucket name');
  process.exit(1);
}
