...
try {
  ...
  const repoNotice = `> **Note**: This package is part of the [MCP Servers](https://github.com/pulsemcp/mcp-servers) monorepo. For the latest updates and full source code, visit the [SVG Tracer MCP Server directory](https://github.com/pulsemcp/mcp-servers/tree/main/productionized/svg-tracer).

`;

  // Insert repo notice after the title
  const titleMatch = mainContent.match(/^#.*SVG Tracer.*$/m);
  if (titleMatch) {
