...
describe('Authentication Health Checks', () => {
  describe('Scraping Service Authentication', () => {
    it('should check scraping service authentication without consuming credits', async () => {
      ...
      if (results.length === 0) {
        console.log(
          '⚠️  No scraping services configured - set FIRECRAWL_API_KEY or BRIGHTDATA_BEARER_TOKEN'
        );
        return;
      }
      ...
      for (const result of results) {
        ...
        if (result.service === 'Firecrawl') {
          ...
          console.log(
            `  🔑 Using BRIGHTDATA_BEARER_TOKEN: ${process.env.BRIGHTDATA_BEARER_TOKEN?.substring(0, 20)}...`
          );
        }
      }
  ...
  describe('Summary', () => {
    it('should provide a summary of all authentication checks', async () => {
      ...
      console.log('  - Scraping: FIRECRAWL_API_KEY, BRIGHTDATA_BEARER_TOKEN');
