...
- **Test Structure**: Integration tests should be split into separate files (`*.source.integration.test.ts`, `*.built.integration.test.ts`) with a main entry point that imports both - this provides flexibility to run tests individually or together
- **Built Mode Testing**: Always simulate the exact CI/CD publish process by running `ci:install` (or `npm install`) followed by `prepare-publish.js` - this ensures tests match what happens during actual npm publishing
- **Test Runner Pattern**: Create a shared test runner function that accepts configuration objects specifying mode name, server path, and optional setup functions - this keeps test logic DRY while allowing different initialization for each mode
- **Timeout Configuration**: Built mode tests require longer timeouts (30s for beforeAll hooks, 60s for overall test timeout) due to the packaging process that includes dependency installation and builds
