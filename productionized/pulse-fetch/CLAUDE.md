...
### Manual Test Enhancement and File I/O

- **Temporary File Management**: When enhancing manual tests to save scraped results, use timestamped filenames with sanitized URLs for easy identification and sorting
- **File Format Selection**: Save text-based results (Native, BrightData) as `.txt` files and structured data (Firecrawl) as `.json` files to preserve formatting
- **TypeScript Linting**: Replace `any` types with `unknown` for better type safety when dealing with dynamic data structures
- **Pre-commit Hook Recovery**: When encountering "Cannot find module" errors in lint-staged, the standard fix is `rm -rf node_modules package-lock.json && npm install` from repo root
- **Gitignore Patterns**: Use `**/tests/manual/temp-results/` pattern to exclude temporary test files from all manual test directories across the monorepo
