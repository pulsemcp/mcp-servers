...
- **Multi-tier Resource Storage**: Resources are now saved in three stages (raw, filtered, extracted) to enable debugging and reuse. FileSystem storage uses subdirectories, Memory storage uses prefixed URIs. The `writeMulti` method ensures atomic writes across all tiers with shared filenames for correlation
